...
if($opt=='a'||$opt=='u'||$opt=='d'){ $q=false;
	...
	if($opt=='uf'||$opt=='df'){ // Prepocessing form
		...
		$t1=mysql_query("SELECT nama FROM pus_pengarang WHERE replid='".$rk['pengarang']."' LIMIT 0,1");
		$r1=mysql_fetch_array($t1);
		$r1=substr($r1['nama'],0,3);
		$r['callnumber'].=" ".$r1;
		...
		$r['idbuku']=buku_getidbuku($lok,$tingb);
		$r['barkode']=buku_idbukutobarkode($r['idbuku']);
		$r['urut']=buku_idbukugeturut($r['idbuku']);
	...
	if($opt=='af' || $opt=='uf'){ require_once(MODDIR.'control.php'); // Add or Edit form
		...
		if($opt=='af'){
		$fform->fi('Jumlah koleksi baru',iText('nbuku',1,'width:30px;text-align:center','','onblur="katalog_buku_cek()"').' &nbsp;item');
