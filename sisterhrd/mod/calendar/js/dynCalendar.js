...
*/
	function dynCalendar_show()
	{
		...
		monthnames = new Array('Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember');
		...
		while(i <= numdays){
            ...
            */
			...
			linkHTML = '<a href="javascript: ' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + '); ' + this.objName + '._hideLayer()">'+ m1 + (i++) + m2 +'</a>';
		...
		prevImgHTML  = '<img src="' + this.imagesPath + '/prev.gif" alt="Bulan Kemarin" border="0" />';
		prevLinkHTML = '<a href="javascript: ' + this.objName + '.show(' + previousMonth + ', ' + previousYear + ')">' + prevImgHTML + '</a>';
		nextImgHTML  = '<img src="' + this.imagesPath + '/next.gif" alt="Bulan Berikutnya" border="0" />';
        ...
        */
		...
		html = '<table border="0" width="100%" style="background:#ffffff">';
...
*/
	function dynCalendar_writeHTML()
	{
		if (is_ie5up || is_nav6up || is_gecko) {
			document.write('<a href="javascript: ' + this.objName + '.show()"><img src="' + this.imagesPath + 'dynCalendar.png" border="0" width="16" height="16" align="absmiddle" /></a>');
			document.write('<div style="font-size:9px;font-family:tahoma;" class="dynCalendar" id="' + this.layerID + '" onmouseover="' + this.objName + '._mouseover(true)" onmouseout="' + this.objName + '._mouseover(false)"></div>');
		}
	}
