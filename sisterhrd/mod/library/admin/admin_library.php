...
if (isset( $_SESSION['LevelAkses'] ) &&  $_SESSION['LevelAkses']=="Administrator"  OR $_SESSION['LevelAkses']=="Editor"){
if($_GET['aksi'] == ''){
$admin .= '
...
while ($file = readdir($rep)) {
	if($file != '..' && $file !='.' && $file !=''&& $file !='Thumbs.db'){
		if (is_dir($file)){
			...
			<td style="text-align:center;padding:2px;border-top:1px solid #ccc;border-left:1px solid #ccc;"valign="top"><a href="files/'.$file.'" target="_blank">View</a></td>
			<td style="text-align:center;padding:2px;border-right:1px solid #ccc;border-top:1px solid #ccc;border-left:1px solid #ccc;"valign="top"><a href="?pilih=admin_files&amp;aksi=hapus&amp;nama='.$file.'"><img src="images/delete.gif"></a></td>
			</tr>';
			$warna = empty($warna) ? 'bgcolor="#efefef"' : '';
			}
		}
...
if ($_GET['aksi']=='hapus'){
	...
	header("location:?pilih=admin_files");
exit;
}
