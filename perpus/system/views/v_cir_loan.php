...
if($keyw!=""){
	$keyw=trim($keyw);
	if(preg_match("/^[a-zA-z ]+$/",$keyw)){ // find member name
		$t=mysql_query("SELECT nip FROM ".DB_HRD." WHERE name LIKE '$keyw %' OR name LIKE '% $keyw' OR name LIKE '% $keyw %' OR name='$keyw' ORDER BY name");
...
if($sortby=='shelf'){
	...
	if($show!='1'){
		$sql="SELECT * FROM loan WHERE status='1' ".$search." ORDER BY ".$sfi.$sm;
		$stablew='1000px';
	} else {
		$sql="SELECT * FROM loan ".$search." ORDER BY member, date1 DESC,date2 DESC";
		$stablew='1000px';
	}
}
if($sf){
...
<?php if($show!='1'){?>
<button class="btn" style="float:left;margin-right:4px;width:146px" onclick="jumpTo('<?=RLNK?>circulation.php?tab=loan&act=view')">
	<div class="bi_lis">View all loan history</div>
</button>
<?php }else{?>
<button class="btn" style="float:left;margin-right:4px;width:131px" onclick="jumpTo('<?=RLNK?>circulation.php?tab=return')">
...
<?php }}?>
	...
	if($show!='1'){
	?>
	<div class="sfont" style="height:26px;margin-top:4px">Books in loan:</div>
	<?php }else{?>
	...
	while($h=mysql_fetch_array($t)){if($rc==0){$rc=1;}else{$rc=0;};
		...
		<tr id="xrow<?=$r['dcid']?>" class="xxr<?=$rc?>">
			...
			<td width="100px">
				<button class="btn" title="View member" style="float:left;margin-right:2px" onclick="jumpTo('<?=RLNK?>members.php?tab=staff&act=view&nid=<?=$h['member']?>')">
