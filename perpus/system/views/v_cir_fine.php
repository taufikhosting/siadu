...
$sql="SELECT * FROM fine ".$search." ORDER BY ".$sfi.$sm;
if($sf){
...
// Paging:
$page_link=RLNK."circulation.php?tab=fine";
// number per page
$npp=30;
// number of page
$nop = ceil($ndata/$npp);
...
<table class="xtable" border="0" cellspacing="0" width="<?=$tablew?>">
	<tr>
		...
		<?=iThxp("Callnumber",'nip',$page,$sortby,$smode,$keyw)?>
...
while($r=dbFAx($t)){ if($n>=$nps && $n<$npl){ if($rc==0){$rc=1;}else{$rc=0;};
	...
	<tr id="xrow<?=$r['dcid']?>" class="xxr<?=$rc?>" <?=$rh?>>
		<td width="*"><?=src_replace($r['member'])?></td>
		<td width="120px"><?=src_replace($r['amount'])?></td>
...
<?php } else { ?>
<div class="sfont">Fine data is currently empty.</div>
<?php }
