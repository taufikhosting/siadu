...
// Paging:
$page_link=RLNK."book.php?tab=manage";
// number per page
$npp=20;
// number of page
$nop = ceil($ndata/$npp);
...
<?php if($keyw==''){?>
	<button class="btn" title="Add new classification" onclick="b_class('af')">
		<div style="background:url('<?=IMGR?>addico.png') no-repeat;padding-left:16px">New book shelf</div>
	</button>
<?php } else {?>
	<div class="sfont">
		<button class="btn" title="Show all classification" onclick="jumpTo('<?=RLNK?>bibliographic.php?tab=class')" style="margin-right:20px">
			<div style="background:url('<?=IMGR?>larrow.png') no-repeat;padding-left:16px">All book shelf</div>
		</button>
	<?php if($ndata>0){ ?>
		Searching <b>"<?=$keyw?>"</b>, found <?=$ndata?> result:
	<?php } else {?>
		<b><?=$keyw?></b> does not match with any book shelf.
	<?php } ?>
	</div>
<?php } ?>
</td>
<td align="right">
	...
	<?=iText('q','',"width:150px",'Search book shelf')?>
	</form>
	<?php }?>
</td>
</tr></table>
...
<table class="xtable" border="0" cellspacing="1px" width="800px">
	<tr>
		...
		<?=iThxp("No. of book",'nbook',$page,$sortby,$smode,$keyw,"text-align:center")?>
...
while($r=mysql_fetch_array($t)){ if($n>=$nps && $n<$npl){ if($rc==0){$rc=1;}else{$rc=0;}; $date1=ftgl($r['date1']); $date2=ftgl($r['date2']);
	...
	<tr id="p_train<?=$r['dcid']?>" class="xr<?=$rc?>">
		...
		<td width="50px"><?=src_replace($r['code'])?></td>
		<td width="*"><?=src_replace($r['name'])?></td>
		<td width="100" align="center"><?=dbSRow("book","W/`shelf`='".$r['dcid']."'")?></td>
		<td width="200"><?=$r['description']?></td>
		<td width="60px" align="center">
			<button class="btnedit" title="Edit" onclick="b_shelf('df',<?=$r['dcid']?>)"></button>
			<?php if($r['dcid']!=1){?><button class="btndel" title="Delete" onclick="b_shelf('df',<?=$r['dcid']?>)"></button><?php }?>
		</td>
	</tr>
<?php } $n++; } ?>
</table>
