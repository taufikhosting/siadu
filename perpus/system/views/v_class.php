...
<?php if(!empty($_SESSION['newentrymsg']) || $keyw!=''){ ?>
	<div id="newemsg" class="sfont" style="text-align:center;padding:2px 6px;background:#f4f4f4;border-radius:5px">
		...
		if(!empty($_SESSION['newentrymsg'])){ // conditional message
			...
			if($ndata>0){ 
				echo 'Searching classification with name <b>"'.$keyw.'"</b>, found '.$ndata.' result:';
			} else {
				echo '<b>'.$keyw.'</b> does not match with any classification name.';
			} 
		}
		?>
	</div>
...
if($keyw=='' && empty($_SESSION['newentry']) ){ // Normal or conditional view option ?>
	<?php if(!$stocktaking){?>
	<button class="btn_fl" title="Add new classification" onclick="cfform('af')">
		<div class="bi_add">New classification</div>
	</button>
<?php }} else {?>
	<button class="btn_fl" title="Show all classification" onclick="jumpTo('<?=RLNK?>bibliographic.php?tab=class')">
		<div class="bi_arrow">All classification</div>
	</button>
...
<?php }}?>
	...
	<div style="float:right;margin-right:4px"><input type="hidden" name="tab" value="class"/>
		<?=iText('q','',"width:180px",'Search classification name')?></div>
	</form>
</div>
