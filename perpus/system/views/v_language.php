...
$sql="SELECT * FROM mstr_language ".$search." ORDER BY ".$sfi.$sm;
if($sf){
...
// Paging:
$page_link=RLNK."bibliographic.php?tab=language";
// number per page
$npp=30;
// number of page
$nop = ceil($ndata/$npp);
...
if(!empty($_SESSION['newentry'])){
	$sql="SELECT * FROM mstr_language WHERE dcid='".$_SESSION['newentry']."' LIMIT 0,1";
	$t=mysql_query($sql);
...
<?php if(!empty($_SESSION['newentrymsg']) || $keyw!=''){ ?>
	<div id="newemsg" class="sfont" style="text-align:center;padding:2px 6px;background:#f4f4f4;border-radius:5px">
		...
		if(!empty($_SESSION['newentrymsg'])){ // conditional message
			...
			if($ndata>0){ 
				echo 'Searching language with name <b>"'.$keyw.'"</b>, found '.$ndata.' result:';
			} else {
				echo '<b>'.$keyw.'</b> does not match with any language name.';
			} 
		}
		?>
	</div>
...
if($keyw=='' && empty($_SESSION['newentry']) ){ // Normal or conditional view option ?>
	<?php if(!$stocktaking){?>
	<button class="btn_fl" title="Add new language" onclick="cfform('af')">
		<div class="bi_add">New language</div>
	</button>
<?php }} else {?>
	<button class="btn_fl" title="Show all language" onclick="jumpTo('<?=RLNK?>bibliographic.php?tab=language')">
		<div class="bi_arrow">All language</div>
	</button>
...
<?php }}?>
	...
	<div style="float:right;margin-right:4px"><input type="hidden" name="tab" value="language"/>
		<?=iText('q','',"width:150px",'Search language name')?></div>
	</form>
</div>
