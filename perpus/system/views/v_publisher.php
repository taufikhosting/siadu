...
if(!empty($_SESSION['newentry'])){
	$sql="SELECT * FROM mstr_publisher WHERE dcid='".$_SESSION['newentry']."' LIMIT 0,1";
	$t=mysql_query($sql);
...
<?php if(!empty($_SESSION['newentrymsg']) || $keyw!=''){ ?>
	<div id="newemsg" class="sfont" style="text-align:center;padding:2px 6px;background:#f4f4f4;border-radius:5px">
		...
		if(!empty($_SESSION['newentrymsg'])){ // conditional message
			...
			if($ndata>0){ 
				echo 'Searching publisher with name <b>"'.$keyw.'"</b>, found '.$ndata.' result:';
			} else {
				echo '<b>'.$keyw.'</b> does not match with any publisher name.';
			} 
		}
		?>
	</div>
...
if($keyw=='' && empty($_SESSION['newentry']) ){ // Normal or conditional view option ?>
	<?php if(!$stocktaking){?>
	<button class="btn_fl" title="Add new publisher" onclick="cfform('af')">
		<div class="bi_add">New publisher</div>
	</button>
<?php }} else {?>
	<button class="btn_fl" title="Show all publisher" onclick="jumpTo('<?=RLNK?>bibliographic.php?tab=publisher')">
		<div class="bi_arrow">All publisher</div>
	</button>
...
<?php }}?>
	...
	<div style="float:right;margin-right:4px"><input type="hidden" name="tab" value="publisher"/>
		<?=iText('q','',"width:150px",'Search publisher name')?></div>
	</form>
</div>
...
while($r=dbFAx($t)){ if($n>=$nps && $n<$npl){ if($rc==0){$rc=1;}else{$rc=0;};
	...
	<tr id="xrow<?=$r['dcid']?>" class="xxr<?=$rc?>" <?=$rh?>>
		...
		<td width="350px" onclick="selectRow(<?=$k?>)"><?=$r['address']?></td>
		<td width="120px" onclick="selectRow(<?=$k?>)"><?=$r['phone']?></td>
		<?php if(!$stocktaking){?>
