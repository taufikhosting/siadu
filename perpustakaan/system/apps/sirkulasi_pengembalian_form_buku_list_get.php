...
$fmod='sirkulasi_pengembalian_form_buku_list';
...
$xtable->select_cekfunc="sirkulasi_pengembalian_form_buku_list_cek(param)";
...
$PSBar->begin();
	if(count($lokasi)>0){
		$PSBar->selection('Lokasi',iSelect('ff2_lokasi',$lokasi,$lok,$PSBar->selws,"pengembalian_buku_get_cari(1)"));
	} else {
		$PSBar->end();
...
$db->field("pus_peminjaman:replid,tanggal1,tanggal2","pus_buku:replid as ibuku,barkode","pus_katalog:judul");
$db->join("buku","pus_buku");
$db->joinother("pus_buku","katalog","pus_katalog");
$db->where_and("pus_buku.lokasi='$lok'");
$db->where_and("pus_peminjaman.status='1'");
$db->where_and("!( NOT EXISTS (SELECT pus_tpjm.replid FROM pus_tpjm WHERE pus_tpjm.peminjaman=pus_peminjaman.replid) )");
...
if($xtable->ndata>0){
	...
	while($r=mysql_fetch_array($t)){$xtable->row_begin($r['replid']);
		...
		if(admin_isoperator()) $s='<button class="btn" onclick="xtable3_cekall(false);xtable3_sel('.$n.');sirkulasi_pengembalian_form_buku_form(\'a\',\'0\',true)">Pilih</button>~40px';
