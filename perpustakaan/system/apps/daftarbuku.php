...
if($opt=='a'||$opt=='u'||$opt=='d'){ $q=false;
	...
	if($opt=='uf'){ require_once(MODDIR.'control.php'); // Add or Edit form
		...
		hiddenval('nbuku',0);
		$fform->fi('Nomor Induk',iText('idbuku',$r['idbuku'],$fform->rwidths,'',''));
		// $fform->fi('Nomor Induk',iText('idbuku',$r['idbuku'],$fform->rwidths,'','','disabled'));
		...
		$fform->fi('Pengarang',iText('pengarang',$r['pengarang'],$fform->rwidths,'','','disabled'));
		$fform->fi('Tahun Terbit',iText('tahunterbit',$r['tahunterbit'],$fform->rwidths));
		$fform->fi('Callnumber',iText('callnumber',$r['callnumber'],$fform->rwidths));
		$fform->fi('Penerbit',iText('penerbit',$r['penerbit'],$fform->rwidths));
