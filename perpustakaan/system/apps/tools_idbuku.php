...
$fmod='tools_idbuku';
$dbtable='pus_setting';
$fform=new fform($fmod,$opt,$cid,'!Format Nomor ID Item');
...
if($opt=='a'||$opt=='u'||$opt=='d'){ $q=false;
	if($opt=='a'){ // add
		...
		if($q && gpost('updatebuku')=='1'){
			...
			while($r1=mysql_fetch_array($t1)){
				...
				$q=dbUpdate("pus_buku",array('idbuku'=>$idbuku),"replid='".$r1['replid']."'");
			}
		}
	}
	...
	if($opt=='af' || $opt=='uf'){ require_once(MODDIR.'control.php'); // Add or Edit form
		...
		$fform->fi('',iCheckx('updatebuku','Update nomor ID item ke format baru',1));
