...
$xtable->btnbar_begin();
	if($SOUF==0) $xtable->btnbar_add('','','Tambah koleksi baru');
...
if($xtable->ndata>0){
	// Table head
	$xtable->head('@Barkode','@!Nomor ID','@sumber','@harga~R','@Tanggal diperoleh','@Status','@lokasi','@tingkat');
	while($r=mysql_fetch_array($t)){$xtable->row_begin($r['replid']);
		...
		if($r['status']==1){
			...
			$s.='<td style="border:none"><span class="sfont">Dipinjam</span></td>';
			$s.='<td style="border:none" width="24px" align="right"><button title="Tampilkan info peminjam" class="btn"><div class="bi_usr2b">&nbsp;</div></button></td>';
			$s.='</tr></table>';
			$s='Dipinjam';
		}
		//$xtable->td($s,100);
		//echo '<td style="width:100px;text-align:left">'.$s.'</td>';
		$xtable->td($s,100);
