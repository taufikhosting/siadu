...
function opac_detil(a){
	alert(a);
	_("katalog_data&cid="+a,function(r){
		alert(r);
...
function buku_warnacover($t){
	$id=intval(ord(substr($t,1,1)))%6;
	$colortheme=Array('#fc9f13','#ff366d','#3ce700','#2694eb','#2546e9','#5a38b3');
...
function photof($d=0,$f='$',$t='',$p=''){
	...
	echo '<div style="position:absolute;bottom:0px;float:left;width:100px;background:#fff" onclick="opac_detil('.$d.')">';
	...
	$nophoto='<div title="'.$t.($p!=''?' by '.$p:'').'" class="imgcoverx" style="box-shadow:0px 2px 10px rgba(0,0,0,0.7);position:relative;cursor:pointer;border-left:8px solid '.buku_warnaborder($t).';width:90px;height:100px;background:'.buku_warnacover($t).';color:#fff;font:11px '.VFONT.';text-align:center;padding:15px 6px"><div style="width:90px;height:100px;overflow:hidden"><b>'.$judul.'</b></div></div>';
...
$db->field("pus_katalog:replid,judul","pus_pengarang:nama as npengarang");
...
<div style="margin-right: 20px; margin-left: 20px;">	
	...
	<table width="100%" cellspacing="0px" cellpadding="0" style="width:840px;margin-top:10px;margin-bottom:20px"><tr valign="top">
		...
		<td><div style="color: #fff;font: 18px <?=SFONT?>;cursor: default;">Perpustakaan</div></td>
	</tr></table>
...
echo '<div style="height:100%;width:100% !important;">';
	...
	if($kol<$nkol && $kol!=0){
		...
		echo '</tr>';
		echo '<tr style="height:20px;background:#eebd46">';
			echo '<td colspan="'.$nkol.'"><span class="sfont" style="box-shadow:0px 2px 4px rgba(0,0,0,0.4);color:#000;background:#fff;padding:2px 4px;border-radius:2px;"><b>'.$fkey.' - '.$lkey.'</b></span></td>';
		echo '</tr>';
		$kol=0;
		$fkey='';
		$row++;
