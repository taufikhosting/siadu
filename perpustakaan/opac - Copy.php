...
<html><head>
<title>SIADU :: OPAC - Perpustakaan</title>
<style type="text/css">
...
.infk{
	position:absolute;top:2px;left:20px;display:inline-block;white-space:nowrap;max-width:110px;height:12px;overflow:hidden;box-shadow:0px 2px 4px rgba(0,0,0,0.4);color:#000;background:#fff;padding:2px 4px;border-radius:2px;font:10px <?=SFONT?>;text-decoration:none;
}
.infk:hover{
...
function opac_detil(a){
	_("opac_detil&cid="+a,function(r){
		//alert(r);
		EHtml('fform',r);open_fform();
	});
}
function opac_setsearchbox(){
	var keyon=E('keyon').value;
...
<?php
function buku_warnacover($t){
	$l=strlen($t);
	$id=intval(ord(substr($t,$l-2,1)))%6;
	$colortheme=Array('#fc9f13','#ff366d','#38d900','#2694eb','#2546e9','#5a38b3');
	return $colortheme[$id];
}
function buku_warnaborder($t){
	$l=strlen($t);
	$id=intval(ord(substr($t,$l-2,1)))%6;
	$colortheme=Array('#be4300','#c60037','#0f7014','#024579','#011887','#24027e');
	return $colortheme[$id];
}
function photof($d=0,$f='$',$t='',$p=''){
	$photodir='../photo/';
	if(empty($d))$d=0;
	echo '<div style="position:relative;width:100px;height:155px;float:left;margin:16px 20px 9px 20px">';
	echo '<div style="position:absolute;bottom:0px;float:left;width:100px" onclick="opac_detil('.$d.')">';
	
	$judul=$t;
	if(preg_match("/^[\w\s\'\-]+/i",$judul,$mat)){
		//$judul=$mat[0];
	}
	$jl=strlen($judul);
	//if($jl<80) $judul='<b>'.$judul.'</b>';
	
	$nophoto='<div title="'.$t.($p!=''?' by '.$p:'').'" class="imgcoverx" style="box-shadow:0px 2px 10px rgba(0,0,0,0.7);position:relative;cursor:pointer;border-left:8px solid '.buku_warnaborder($t).';width:90px;height:100px;background:'.buku_warnacover($t).';color:#fff;font:'.($jl>60?10:11).'px '.VFONT.';text-align:center;padding:15px 6px"><div style="width:90px;height:100px;overflow:hidden">'.$judul.'</div></div>';
	
	if($d!=0 && file_exists($photodir.$f.'.php')){
		$t1=mysql_query("SELECT photo FROM pus_katalog WHERE replid='$d'");
		if(mysql_num_rows($t1)>0){
			$r=mysql_fetch_array($t1);
			if(strlen($r['photo'])>0){
				$sz='width="100px"';
				echo '<img title="'.$t.($p!=''?' by '.$p:'').'" class="imgcover" style="box-shadow:0px 2px 10px rgba(0,0,0,0.7);position:relative;cursor:pointer;float:left" src="'.$photodir.$f.'.php?id='.$d.'" '.$sz.' style="display:"/>';
			}
			else {
				echo $nophoto;
			}
...
$keyw=gets('keyword');
$keysel=gets('keysel');
$keyon=gets('keyon','judul');
$urut=gets('urut','az');
$db=new xdb("pus_katalog");
$db->field("pus_katalog:replid,judul,tahunterbit,klasifikasi","pus_pengarang:nama as npengarang","pus_klasifikasi:kode,nama as nklas");
$db->join("pengarang","pus_pengarang");
$db->join("klasifikasi","pus_klasifikasi");

if(!(empty($keyw) && empty($keysel))){
	if($keyon=='judul'){
		$db->where_and("pus_katalog.judul LIKE '%".$keyw."%'");
		$keysel=0;
	}
	else if($keyon=='pengarang'){
		$db->where_and("pus_pengarang.nama LIKE '%".$keyw."%'");
		$keysel=0;
	}
	else if($keyon=='klasifikasi'){
		if($keysel!=0){
			$db->where_and("pus_klasifikasi.replid='".$keysel."'");
		}
		$keyw='';
	}
}
if($keyon=='pengarang'){
	$db->where_and("pus_pengarang.replid<>0");
	$db->order("pus_pengarang.nama");
} else if($keyon=='klasifikasi'){
	$db->where_and("pus_klasifikasi.replid<>0");
	$db->order("pus_klasifikasi.kode");
}
if($urut=='baru'){
	$db->order("pus_katalog.tahunterbit DESC");
}
$db->order("pus_katalog.judul");
$t=$db->query();
$ndata=mysql_num_rows($t);

?>
<div style="margin-right: 20px; margin-left: 20px;">	
	<table width="100%" cellspacing="0px" cellpadding="0" style="margin:auto"><tbody><tr><td align="center">
	<table width="100%" cellspacing="0px" cellpadding="0" style="width:840px;margin-top:10px;margin-bottom:20px"><tr valign="top">
		<td style="" width="100px"><div style="height: 32px;width: 85px;background: url('<?=IMGR?>siadu_32.png') no-repeat;"></div></td>
		<td><div style="color: #fff;font: 18px <?=SFONT?>;cursor: default;">OPAC - Perpustakaan</div></td>
	</tr></table>
	<div style="width:840px;height:30px;">
			<?php $t1=mysql_query("SELECT * FROM pus_klasifikasi ORDER BY kode");
			$klasifikasi_num=mysql_num_rows($t1);
			$klasifikasi=array();
			$klasifikasi[0]='- Semua -';
			while($r1=mysql_fetch_array($t1)){
				$klasifikasi[$r1['replid']]=$r1['kode'].' '.$r1['nama'];
			}
			hiddenval('klasifikasi_num',$klasifikasi_num);
			$keyons=array('judul'=>'Judul','pengarang'=>'Nama pengarang');
			if($klasifikasi_num>0) $keyons['klasifikasi']='Klasifikasi';
		
		/*
		echo '<div class="sfont" style="color:#fff;float:right;margin-left:4px;margin-right:4px;box-shadow:0px 2px 2px rgba(0,0,0,0.6);border-radius:4px;background:rgba(0,0,0,0.2)">';
		echo '<a style="float:right;display:block;border-radius:0px 4px 4px 0px;padding:4px 6px;'.($urut=='baru'?'background:rgba(0,50,128,0.8)':'').'" title="Urut dari yang terbaru" href="opac.php?keyword='.$keyw.'&keysel='.$keysel.'&keyon='.$keyon.'&urut=baru" class="linkw">Terbaru</a>';
		echo '<a style="float:right;display:block;border-radius:4px 0px 0px 4px;padding:4px 6px;'.($urut!='baru'?'background:rgba(0,50,128,0.8)':'').'" title="Urut berdasarkan abjad" href="opac.php?keyword='.$keyw.'&keysel='.$keysel.'&keyon='.$keyon.'&urut=az" class="linkw">A-Z</a>';
		//echo '&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;';
		//echo '&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a title="Urut dari yang terpopuler" href="opac.php?urut=populer" class="linkw">Terpopuler</a>';
		echo '</div>';
		*/
		echo '<div class="sfont" style="color:#fff;float:right;margin-right:4px;margin-top:4px;margin-right:4px"><a title="Urut berdasarkan abjad" href="opac.php?keyword='.$keyw.'&keysel='.$keysel.'&keyon='.$keyon.'&urut=az" class="linkw">A-Z</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;';
		
		echo '<a title="Urut dari yang terbaru" href="opac.php?keyword='.$keyw.'&keysel='.$keysel.'&keyon='.$keyon.'&urut=baru" class="linkw">Terbaru</a>';
		//echo '&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a title="Urut dari yang terpopuler" href="opac.php?urut=populer" class="linkw">Terpopuler</a>';
		echo '</div>';
			?>
		<form id="search_form" action="opac.php" method="get" style="margin:0;padding:0">
		<?php 
			$ffun='opac_cari()';
			echo '<div class="sfont" style="color:#fff;float:left;margin-right:4px;margin-top:4px;margin-right:4px">Cari berdasarkan:</div>';
			echo iSelect('keyon',$keyons,$keyon,'float:left;margin-right:4px;margin-right:4px','opac_setsearchbox()');
			echo iText('keyword',gets('keyword'),'width:200px;float:left;margin-right:4px;display:'.($keyon=='klasifikasi'?'none':''),'...','','onkeyup=""');
			echo iSelect('keysel',$klasifikasi,$keysel,'display:'.($keyon=='klasifikasi'?'':'none').';float:left;margin-right:4px');
			echo '<button title="Cari" class="btn" style="float:left;margin-right:4px" onclick="sirkulasi_peminjaman_form_buku_list_open()"><div class="bi_srcb">&nbsp;</div></button>';
			//echo iTextSrc('keyword',gets('keyword'),'float:left~display:'.($keyon=='klasifikasi'?'none':'').';width:200px','...',$ffun,'onkeyup="gpage_cari(event,function(){'.$ffun.'})"');
	...
	<div style="width:840px;height:30px;">
		<div style="width:100%">
			...
			if(!(empty($keyw) && empty($keysel))){
				if($keyon=='pengarang') {
					$ikeyon='nama pengarang'; $ikeyw=stripslashes($keyw);
				} else if($keyon=='klasifikasi') {
					$ikeyon='klasifikasi'; $ikeyw=$klasifikasi[$keysel];
				} else {
					$ikeyon='judul'; $ikeyw=$keyw;
				}
				if($ndata>0){
					$info='Hasil pencarian koleksi dengan '.$ikeyon.'  "<b>'.$ikeyw.'</b>". Ditemukan '.$ndata.' item.';
				} else {
					$info='Tidak ditemukan koleksi dengan '.$ikeyon.'  "<b>'.$ikeyw.'</b>".';
				}
				echo '<div class="infobox" style="border-radius:4px;background-color:#fff;box-shadow:0px 1px 4px rgba(0,0,0,0.5);color:#">';
					echo $info;
				echo ' <a href="opac.php?keyon='.$keyon.'&urut='.$urut.'" class="linkb" style="color:"><button class="btn">Tampilkan semua...</button></a></div>';
			}
			...
			<?php
function buku_judul2($a){
	$b=explode("`",$a);
	$c=$b[0];
	return stripslashes($c);
}

$row=0; $kol=0; $nkol=5; $fkey=''; $lkey=''; $nb=0; $itahun=array(); $ijudul=array();
...
echo '<div style="height:100%;width:100% !important;">';
	...
	while($r=mysql_fetch_array($t)){
		$itahun[$kol]=$r['tahunterbit'];
		if($keyon=='pengarang') {
			$ijudul[$kol]=$r['npengarang'];
			$ireplid[$kol]=$r['npengarang'];
		} else if($keyon=='klasifikasi') {
			$ijudul[$kol]=$r['kode'].' '.$r['nklas'];
			$ireplid[$kol]=$r['klasifikasi'];
		} else {
			$ijudul[$kol]=buku_judul2(str_firstword($r['judul']));
		}
		...
		if($kol==0){
			...
			if($keyon=='pengarang') {
				...
				$fkey=buku_judul2(str_firstword($r['judul']));
			}
				...
				if($keyon=='pengarang'||$keyon=='klasifikasi'){
					if($urut=='baru' || true){
						...
						for($it=0;$it<$nkol;$it++){
							...
							if($lt!=$itahun[$it] && $urut=='baru' && $itahun[$it]!=''){
								if($infk!='')$infk.=', ';
								$infk.=$itahun[$it];
							}
							...
							if($keyon=='pengarang'){
								$ttl='Tampilkan koleksi dari '.$ijudul[$it];
								$lnk='opac.php?keyon=pengarang&keyword='.$ireplid[$it];
							} else if($keyon=='klasifikasi'){
								$ttl='Tampilkan koleksi dengan klasifikasi '.$ijudul[$it];
								$lnk='opac.php?keyon=klasifikasi&keysel='.$ireplid[$it];
							}
							
							if($infk!=''){
								echo '<a title="'.$ttl.'" href="'.$lnk.'" class="infk"><b>'.$infk.'</b></a>';
							}
							echo '</td>';
							$lt=$itahun[$it];
							$lj=$ireplid[$it];
						...
						for($it=0;$it<$nkol;$it++){
							echo '<td style="position:relative">';
							if($lt!=$ijudul[$it]){
								echo '<a class="infk" style="cursor:default"><b>'.$ijudul[$it].'</b></a>';
							}
							echo '</td>';
							$lt=$ijudul[$it];
						}
					...
					if($urut=='baru'){
						...
						for($it=0;$it<$nkol;$it++){
							echo '<td style="position:relative">';
							if($lt!=$itahun[$it]){
								echo '<a class="infk" style="cursor:default"><b>'.$itahun[$it].'</b></a>';
							}
							echo '</td>';
							$lt=$itahun[$it];
						}
					} else {
						echo '<td colspan="'.$nkol.'" style="position:relative">';
							echo '<a class="infk" style="cursor:default"><b>'.$fkey.' - '.$lkey.'</b></a>';
						echo '</td>';
					}
				}
			echo '</tr>';
	...
	if($kol<$nkol && $kol!=0){
		//print_r($itahun);
		for($i=$kol;$i<$nkol;$i++){
			echo '<td><div style="position:relative;width:100px;height:155px;float:left;margin:16px 20px 9px 20px"></div></td>'; $nb++;
			$itahun[$i]='';
			$ijudul[$i]='';
		}
		echo '</tr>';
