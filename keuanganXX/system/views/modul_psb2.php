<?php 
	...
	if(count($departemen)>0){
		...
		if(mysql_num_rows($t)>0){
			...
			$sql = "SELECT psb_calonsiswa.replid,psb_kelompok.biaya FROM psb_calonsiswa LEFT JOIN psb_kelompok ON psb_kelompok.replid=psb_calonsiswa.kelompok WHERE psb_calonsiswa.proses='$pros'";
			$t   =mysql_query($sql);

			while($r=mysql_fetch_array($t)){
				$sql2 = "SELECT * FROM keu_pembayaran WHERE modul='$modid' AND siswa='".$r['replid']."'";
				// var_dump($sql2);exit();
				$t1=mysql_query($sql2);
		...
		// Tabel transaksi
			...
			// function iSelect($id,$val,$selected,   $y='',$cb='',$atr='',$req=''){
			// $t.=iSelect('tglTB',
			// 			array(
			// 				'1' => 'a',
			// 				'2' => 'b',
			...
			$xtable->btnbar_f('print','srcbox',$s,$t);
			// echo 'ok';
			...
			if($xtable->ndata>0){
				// Table head
				$xtable->headCB('@No pendaftaran','@Nama','Biaya pendaftaran{R}','Jumlah dibayar{R}','@Tanggal pembayaran','@Status');
