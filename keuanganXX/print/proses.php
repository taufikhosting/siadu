...
// Queries:
$t=mysql_query("SELECT * FROM psb_proses WHERE departemen='$dept' ORDER BY proses");
...
$pdf->MultiCell($dcPageW, 0, 'DATA PROSES PENERIMAAN SISWA BARU', 0, 'C', 0, 1, '', '', true);
dc_YDown(3);
...
$pdf->MultiCell(100, 0, ': '.$departemen['departemen'], 0, 'L', 0, 0, '', '', true);
$pdf->Ln();

dc_YDown(2);
$thx=Array('No.','Periode','Kode Awalan','Jumlah','Keterangan');
$twx=Array(   11,      40,           30,      20,           0);
$tax=Array(  'C',     'L',          'L',     'C',         'L');
...
while($r=dbFA($t)){ $i=0;
	...
	$pdf->MultiCell($twx[$i], 0, $r['proses'], 0, $tax[$i++], 0, 0, '', '', true);
	if($pdf->getLastH()>$my)$my=$pdf->getLastH();
	
	$pdf->MultiCell($twx[$i], 0, $r['kodeawalan'], 0, $tax[$i++], 0, 0, '', '', true);
	if($pdf->getLastH()>$my)$my=$pdf->getLastH();
	
	$sql = "SELECT replid FROM psb_calonsiswa WHERE proses = '".$r['replid']."' AND aktif = 1";
	$q = mysql_query($sql);
	$n = mysql_num_rows($q);
