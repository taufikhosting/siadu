<?php 
	...
	if(count($departemen)>0){
		...
		$s="SELECT 
				...
				k.nip,
				k.nama,
				p.nama as namapelajaran 
			FROM 
				aka_guru g
				LEFT JOIN hrd_karyawan k ON g.pegawai=k.id 
		...
		if($pel==0){ // tampilkan semua PELAJARAN
			$t=mysql_query($s);
		...
		} else { // tampilkan pelajaran tertentu 
			$s2='SELECT 
					g.*,
					k.nip,
					k.nama 
