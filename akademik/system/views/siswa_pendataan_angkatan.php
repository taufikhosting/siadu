<?php 
	...
	if(count($departemen)>0){
		...
		$PSBar = new PSBar_2();
		$PSBar->begin();
		$PSBar->selection_departemen($fmod,$dept); //combo departemen

		if(count($angkatan)>0){
			$PSBar->selection('Angkatan',iSelect('angkatan',$angkatan,$angk,$PSBar->selws,$fmod."_get()"));  // combo angkatan 
		} else {
			$PSBar->end();
		...
		if($PSBar->pass){
			if($opt=='af'||$opt=='uf') 
				...
				if($xtable->ndata>0){
					...
					$xtable->nodata_cust('Tidak ada data siswa pada angkatan ini.');
				}
			}
		}
	} else { 
