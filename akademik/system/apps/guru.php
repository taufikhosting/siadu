<?php 
	...
	if($opt=='a'||$opt=='u'||$opt=='d'){ //db operation : add , update, delete 
		$q=false;
		if($opt=='a'){ // add
		...
		if($opt=='uf'||$opt=='df'){ // db operation for form : update , delete
			$r=dbSFA("*",$dbtable,"W/replid='$cid'");
			$r['departemen']=gpost('departemen');
			$s='SELECT * FROM hrd_karyawan WHERE id='.$r['pegawai'].' LIMIT 0,1';
			// var_dump($s);exit();
			$q=mysql_query($s);
		...
		} else { // db operation for form : add
