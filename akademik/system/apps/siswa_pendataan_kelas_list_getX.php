<?php 
	...
	appmod_use('aka/angkatan','aka/siswa'); // required(path); 
	...
	if($PSBar->pass){
		...
		if($xtable->ndata>0){
			...
			while($r=mysql_fetch_array($t)){
				$xtable->row_begin($r['replid']); //centang 

				$xtable->td($r['nis'],100);
				$xtable->td($r['nama']);
				
				if(admin_isoperator()) 
					$s='<button class="btn" onclick="xtable2_cekall(false);xtable2_sel('.$n.');siswa_pendataan_kelas_siswa_form(\'a\',\'0\',true)"">Pilih</button>~40px';
				else 
