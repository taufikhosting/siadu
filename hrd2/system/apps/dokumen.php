...
if($opt=='a'||$opt=='u'||$opt=='d'||$opt=='m'){ $q=false;
	...
	if($opt=='af' || $opt=='uf'){ $okbtn=($opt=='af')?"Simpan":"OK"; $nobtn=" Batal "; $gd='true'; // Add or Edit form 
		...
		app_form_fi('Pengingat',iText('reminder',$r,'width:40px'),'hari');
		...
		$dcfm='Data dokumen pada pegawai juga akan dihapus.';
		app_form_dlgw($dcf,$dcfm);
