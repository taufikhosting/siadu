...
if($opt=='a'||$opt=='u'||$opt=='d'||$opt=='up'||$opt=='dn'){
	...
	<div class="fformbox" style="width:<?=($fwidth+20)?>px">
		...
		<?php if($opt=='af' || $opt=='uf'){ $okbtn = ($opt=='af')?"Save":"OK"; $gd='true';
			// Add or Edit form ?>
			<tr><td width="100px">Document name:</td><td><?=iText('name',$r['name'],$iTextFw)?></td></tr>
		<?php } else if($opt=='df'){ $okbtn = "Yes"; $nobtn="   No   "; $gd='false';
			// Delete form ?>
			<tr><td><p class="line150">Are you sure you want to delete "<b><?=$r['name']?></b>"? This also delete all "<?=$r['name']?>" of the employee.</p></td></tr>
		<?php }?>
			</table>
