...
if($opt=='u'||$opt=='rf'){
	if($opt=='u'){
		...
		foreach($mstr_status as $cid=>$val){
			$rem=gpost('remstat'.$cid);
	...
	<div class="fformbox" style="width:<?=($fwidth+20)?>px">
		...
		<?php $okbtn = ($opt=='af')?"Save":"OK"; $gd='true'; // Edit form ?>
			<?php $inparray=""; $n=0; while($r=dbFA($t)){if($r['dcid']!=1){ if($n==0) $n=1; else $inparray.="-"; $inparray.="remstat".$r['dcid'];?>
			<tr><td width="<?=$lwidth?>px"><?=$r['name']?>:</td><td><?=iText('remstat'.$r['dcid'],$r['reminder'],"width:40px;text-align:center")?> day(s)</td></tr>
			<?php }}?>
			</table>
