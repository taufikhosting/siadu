	...
	else
	{
		if($_FILES["file"]["size"] < 2000000){
			...
			else
			{
				...
				mysql_query("INSERT INTO tmp_photo SET photo='$imagedata'");
...
<script type="text/javascript" language="javascript">
	function remfile(){
		parent.getPhoto('',0,<?=$rs?>,'<?=$rel?>');
		document.location='trform.php?ow=<?=$ow?>&oh=<?=$oh?>&rel=<?=$rel?>';
	}
</script>
</head>
...
<div onclick="remfile()" class="atch" title="<?=$title?>"><?=$fname?> <button class="rembtn" style="float:right"></button></div>
<?php } else {?>
...
<script type="text/javascript" language="javascript">
<?php if($cerr==0){?>
	parent.getPhoto("<?=$filename?>",<?=$imageid?>,<?=$ns?>,'<?=$rel?>');
<?php } else {?>
alert('<?=$errormsg?>');
