...
class xtable{
	...
	function begin(){
		...
		hiddenval('xtable_page_sort',$this->page_sort);
		hiddenval('xtable_page_sort_dir',$this->page_sort_dir);
		hiddenval('xtable_page_number',$this->page_number);
		hiddenval('xtable_page_search',$this->page_search);
	...
	function head_multi(){
		...
		for($hi=0;$hi<$this->head_rnum;$hi++){
			...
			if($this->useselect && $frow){
				echo '<th rowspan="'.$rs.'" width="1" style="text-align:center"><input id="xtcekt" type="checkbox" onclick="xtable_cekall(\''.$this->fmod.'\',this.checked)" /></th>';
			}
	...
	function head_thf($k,$sr=''){
		$thf=array('title'=>'','width'=>'*','align'=>'l','sortby'=>'','sort'=>false);
	...
	function head_th($t){
		...
		if($sort){
			if(gpost('page_sort')==($sr)){
				echo '<th rowspan="'.$rs.'" colspan="'.$cs.'" width="'.$hw.'" class="sort'.gpost('page_sort_dir').'" title="Urut berdasarkan '.$l.'." style="text-align:'.$al.';'.($al=='right'?'padding-right:15px':'').'" onclick="page_sort='.$sr.';page_sort_dir=\''.(gpost('page_sort_dir')=='ASC'?'DESC':'ASC').'\';'.$this->fmod.'_get()">'.$ttl.'</th>';
			} else {
				echo '<th rowspan="'.$rs.'" colspan="'.$cs.'" width="'.$hw.'" class="sort0" title="Urut berdasarkan '.$l.'." style="text-align:'.$al.'" onclick="page_sort='.$sr.';page_sort_dir=\'ASC\';'.$this->fmod.'_get()">'.$ttl.'</th>';
			}
		}
		else {
			echo '<th rowspan="'.$rs.'" colspan="'.$cs.'" width="'.$hw.'" style="text-align:'.$al.'">'.$ttl.'</th>';
		}
	}
	...
	function foot(){
		...
		echo '</table></div>';
		hiddenval($this->fmod.'xtceknum',$this->row_num);
		hiddenval($this->fmod.'xtable_selectedid','');
		// pagging
		...
		if($this->useselect!=''){
			...
			echo '<td><div style="float:left;height:26px;margin-top:4px"><div id="'.$this->fmod.'xtable_cek_opt" style="display:none">'.$this->select_opt.'</div></div></td>';
		}
		if($this->ndata>$this->rpp && $this->usepaggging){ $fp=true;
			...
			echo '<td width="40px"><span class="sfont">Halaman:</span></td>';
				...
				if($page>1) echo '<a class="plink" title="Halaman sebelumnya" href="#&'.$this->fmod.'" onclick="E(\''.$this->fmod..'xtable_page_number\').value='.($page-1).';'.$this->fmod.'_get()"><</a>';
				else echo '<a class="plinko" href="#&'.$this->fmod.'"><</a>';
				echo '</td>';
				
				for($n=1;$n<=$np;$n++){
					if(($n>=($page-3) && $n<=($page+3)) || $n==$np || $n==1){ $fp=true;
						echo '<td width="24px" align="center"><a onclick="E(\''.$this->fmod.'xtable_page_number\').value='.$n.';'.$this->fmod.'_get()" class="plink'.(($n!=$page)?'" title="Halaman '.$n.'" href="#&'.$this->fmod.'"':'a').'">'.$n.'</a></td>';
					}
					else if($fp){ $fp=false;
				...
				echo '<td width="24px" align="center">';
				if($page<$np) echo '<a class="plink" title="Halaman berikutya" href="#&'.$this->fmod.'" onclick="E(\''.$this->fmod.'xtable_page_number\').value='.($page+1).';'.$this->fmod.'_get()">></a>';
				else echo '<a class="plinko" href="#&'.$this->fmod.'">></a>';
				echo '</td>';
