...
$jumlah = $koneksi_db->sql_numrows( $total );	
if ($jumlah) $error .= "Error: Kode Golongan $nip sudah ada di dalam database! , harap memakai kode lain<br />";
	if ($error){
		$admin .= '<div class="error">'.$error.'</div>';
	}else{
		$hasil  = mysql_query( "INSERT INTO `hrd_golongan` (`nama`,`gajipokok`) VALUES ('$nama','$gajipokok')" );
		if($hasil){
			$admin .= '<div class="sukses"><b>Berhasil di Buat.</b></div>';
		}else{
			$admin .= '<div class="error"><b> Gagal di Buat.</b></div>';
...
$nama     		= !isset($nama) ? '' : $nama;
$gajipokok     		= !isset($gajipokok) ? '' : $gajipokok;
$admin .= '<div class="panel panel-info">
<div class="panel-heading"><h3 class="panel-title">Tambah</h3></div>';
...
<table border="0" cellspacing="0" cellpadding="0"class="table table-condensed">
	...
	<tr>
		<td>Gaji Pokok</td>
		<td>:</td>
		<td><input type="text" name="gajipokok" size="25"class="form-control"></td>
	</tr>
	<tr>
		<td></td>
		<td></td>
...
if(isset($_POST['submit'])){
	...
	if ($error){
		...
		$hasil  = mysql_query( "UPDATE `hrd_golongan` SET `nama`='$nama',`gajipokok`='$gajipokok' WHERE `id`='$id'" );
		if($hasil){
			$admin .= '<div class="sukses"><b>Berhasil di Update.</b></div>';
