		...
		if($xtable->ndata>0){
			...
			while($r=mysql_fetch_array($t)){
				$xtable->row_begin();
					...
					$xtable->td('<b><span style="color:'.$cl.'">'.$r['nomer'].'<br/><span style="color:#444">'.jt_jenisbukti($r['jenis']).'</span>'.($r['nobukti']==''?'':'<br/><span style="color:#444">'.$r['nobukti'].'</span>').'</span></b>',120);
					...
					if(mysql_num_rows($t1)>0){
						...
						while($r1=mysql_fetch_array($t1)){
							$s.='<tr>
									<td>'.$r1['nrek'].'</td>
									<td width="40px" align="center">'.$r1['koderek'].'</td>
									<td width="100px" align="right">'.fRp($r1['debet']).'</td>
									<td width="100px" align="right">'.fRp($r1['kredit']).'</td>
								</tr>';
						}$s.='</table></div>';
					} else {
						$s='';
