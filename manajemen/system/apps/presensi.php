...
appmod_use('aka/siswa','aka/kelas','aka/tingkat','aka/tahunajaran');
$ssid=session_id();
...
if($opt=='u'){ $q=false;
	...
	if($opt=='af'){
		...
		$t=$db->query(); $r=mysql_fetch_array($t);
		$fform->fl('Nama',$r['nama']);
		$fform->fl('Kelas',$r['nkelas']);
		$fform->fl('Bulan',ftgl_namabulan(intval(date("m",$tanggal))));
	}
	$fform->rheight='30px';
	echo '<tr><td colspan="2"><div id="box_presensi_list">';
		require_once(APPDIR.'presensi_list_get.php');
	echo '</div></td></tr>';
	$fform->foot();
}
?>